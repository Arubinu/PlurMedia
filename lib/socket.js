...
var init = ( app, db, tmp_path, functions ) => {
	...
	sio.on( 'connection', ( socket ) => {
		...
		socket.on( 'open', ( data ) => {
			...
			try
			{
				...
				if ( result )
				{
					...
					if ( media )
					{
						...
						{
							//method = 1;
							media = socket.request.headers.referer + [ 'media', data.id, pathlib.basename( media ) ].join( '/' );
						}
			...
			{
				error = true;
				functions.error( __filename, 'open', err );
			}
		...
		socket.on( 'medias', ( data ) => {
				...
				if ( category )
				{
					...
					if ( typeof( data.search ) === 'string' )
					{
						for ( word in data.search.split( ' ' ) )
						{
							...
							if ( search )
								search += ' OR ';
							search += '( ( `name` || `type` || `date` || `directed_by` || `written_by` || `studio` || `actors` || `description` ) LIKE ( \'%\' || ? || \'%\' ) )';
						}
					}
			...
			{
				error = true;
				functions.error( __filename, 'medias', err );
			}
		...
		socket.on( 'infos', ( data ) => {
			...
			{
				error = true;
				functions.error( __filename, 'infos', err );
			}
